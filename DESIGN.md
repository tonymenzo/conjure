...
Combinator takes the opposite move. Two primitives — **recursive agent
...
**Combinator:** Agent *specs* (their role prompt, capabilities, allowed
...
**Combinator:** A spawn prompt is exactly this — the task description plus
...
**Combinator:** This is literally the mailbox. "Don't return to me — send
...
**Combinator:** If a child's reply doesn't need to come back to its
...
**Combinator:** When agents share a document or plan, every edit produces
...
**Combinator:** Spawn an agent but don't actually start it until someone
...
**Combinator:** "Run the editor agent on its own output until it makes no
...
- **Tasks where the model's own context window is the right coordination
  ...
  with a well-structured prompt — no spawn needed. Combinator does not
