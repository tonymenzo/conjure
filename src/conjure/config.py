"""Declarative configuration for ``conjure run``.
...
.. code-block:: yaml
    ...
    root:
      role_prompt: |
        You are the root agent of this conjure session.
      engine: orchestral
      llm: default
...
class RootConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")
    role_prompt: str
    # See ``conjure.engines.resolve_engine_name`` for ``"auto"``
