"""``conjure-input`` — the interactive prompt that lives in window 0
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="conjure-input")
    parser.add_argument(
        "--input-path",
        type=Path,
        required=True,
    ...
    console.print(
        f"[dim]conjure input →[/] [bold magenta]{args.label}[/]"
    )
    console.print(
