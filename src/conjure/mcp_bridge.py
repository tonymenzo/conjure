"""``conjure-mcp`` — stdio MCP server that exposes conjure's
orchestration tools (the spawn primitive, send, recv, agent_map, ...) to the
...
Architecture:
  ...
  claude subprocess
       │  MCP stdio
       ▼
  conjure-mcp (this script)
       │  Unix socket
       ▼
  conjure daemon (control.tool_call)
       │
       ▼
  conjure.tools.{primitives,combinators} ← actual runtime work

The bridge is intentionally thin: each MCP tool inherits the
corresponding conjure tool's field declarations (so the SDK sees
...
Required env vars:
  CONJURE_TOKEN   — caller's runtime_token (per-agent identity).
  CONJURE_SOCKET  — daemon control socket path.
"""
...
from conjure.control import ControlClient
...
# Map each MCP tool's short name to (conjure tool class,
...
# exposes to the LLM (mcp__conjure__<DisplayName>).
#
...
def _make_bridge_class(
    ...
    bridge = type(
        ...
        (target_cls,),
        {
            ...
            + "\n\n(Bridged: forwards to the conjure daemon over MCP.)",
...
def _build_bridge_tools(token: str, socket_path: str) -> list:
    """Instantiate one bridge tool per conjure tool, bound to
...
def main(argv: Sequence[str] | None = None) -> int:
    token = os.environ.get("CONJURE_TOKEN", "")
    socket_path = os.environ.get("CONJURE_SOCKET", "")
    if not token or not socket_path:
        print(
            "conjure-mcp requires CONJURE_TOKEN and CONJURE_SOCKET "
    ...
    if not Path(socket_path).exists():
        print(
            f"conjure-mcp: control socket not found: {socket_path}",
            file=sys.stderr,
        )
        return 2
    ...
    server = MCPServer(
        tools=tools,
        name="conjure",
        # PascalCase display names so the conjure tools read the
