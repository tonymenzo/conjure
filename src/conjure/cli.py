"""``conjure`` CLI entry point.

Subcommands:

- ``conjure run <config>``   — tmux-native mode: each agent gets a
...
- ``conjure run --attach``   — attach to the most-recent
                                  ``conjure-*`` tmux session.
- ``conjure run --attach <name>`` — attach to a specific session.
- ``conjure repl <config>``  — single-pane REPL mode (no tmux);
                                  useful for scripting and tests.
- ``conjure check <config>`` — validate config + report key
                                  availability.
- ``conjure config list|set|unset`` — manage the user .env.

REPL control commands (``conjure repl`` only):
...
from conjure import _ui
from conjure.address import USER, Address
from conjure.config import load_config
from conjure.control import ControlServer
from conjure.daemon import (
    daemonize,
    is_daemon_running,
    list_session_names,
    log_path_for,
...
from conjure.event_log import EventLog
from conjure.events import make_system_prompt_event
from conjure.profiling import profile_session
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="conjure")
    ...
    run_p.add_argument(
        ...
        default=None,
        metavar="SESSION",
        help="Attach to an existing conjure session (newest if name omitted).",
    )

    quit_p = sub.add_parser(
        "quit",
        help="Stop a running conjure daemon (newest if name omitted).",
    )
    quit_p.add_argument(
        "session",
        nargs="?",
        default=None,
        help="Session name (default: newest conjure-* daemon).",
    )
...
# ----- conjure run (tmux orchestrator) ------------------------------------

def _cmd_run_tmux(config_path: Path) -> int:
    """Start a daemonized conjure runtime, set up the tmux session
    with one window per agent, then attach the user's terminal.

    Lifecycle:

    - Parent (the original ``conjure run`` invocation) picks a
    ...
    ``conjure run --attach``. Stop the daemon with ``conjure quit``.
    """
    from conjure.env import load_env_files
    ...
    session_name = f"conjure-{uuid.uuid4().hex[:6]}"
    ...
    if daemon_pid > 0:
        ...
        _ui.print_system(
            ...
            f"[dim]`conjure quit` to stop the daemon[/])",
        )
        time.sleep(0.1)
...
def _run_daemon(*, cfg, session_name: str, pid_path: Path) -> int:
    ...
    Tmux layout is a single window (``conjure-main``) hosting the
    ...
    When ``CONJURE_PROFILE`` is set the whole session is wrapped in
    cProfile and the stats are written out on shutdown — see
    ``conjure.profiling`` for the env var contract.
    """
...
def _run_daemon_inner(*, cfg, session_name: str, pid_path: Path) -> int:
    ...
    store_dir = Path(cfg.runtime.store_dir or "./.conjure/store")
    session_dir = store_dir / "sessions" / session_name
    agents_dir = session_dir / "agents"
    agents_dir.mkdir(parents=True, exist_ok=True)

    # Resolve absolute path to conjure-main so tmux's shell finds it
    # regardless of the user's login-shell PATH (conda envs are rarely
    # on the login PATH).
    import shutil as _shutil
    main_bin = _shutil.which("conjure-main") or "conjure-main"
    ...
    # Single tmux window (conjure-main) hosts everything. Dedicated
...
def _bind_files_popup(session_name: str) -> None:
    ...
    files_path = shutil.which("conjure-files") or "conjure-files"
...
def _cmd_run_attach(session: str) -> int:
    ...
    if session == "__most_recent__":
        ...
        if target is None:
            print("no conjure-* tmux session found", file=sys.stderr)
            return 1
        session = target
...
def _find_most_recent_spawn_session() -> str | None:
    """Return the name of the newest live conjure daemon session."""
    ...
    matching = [
        ...
        if (s.session_name or "").startswith("conjure-")
    ]
    if not matching:
        return None
    matching.sort(
...
def _cmd_quit(session: str | None) -> int:
    console = _ui.make_console()
    if session is None:
        session = _find_most_recent_spawn_session()
        if session is None:
            _ui.print_error(console, "no conjure daemon found")
            return 1
...
def _cmd_check(config_path: Path) -> int:
    from conjure.env import load_env_files
    from conjure.llm import api_key_present, key_env_for
    ...
    console.print(f"[dim][conjure][/dim] config: [cyan]{config_path}[/]")
    ...
    console.print()
    console.print("[dim][conjure][/dim] LLMs:")
    missing = []
    ...
    if missing:
        ...
        console.print(
            "  [dim]Set them in your shell, in ~/.config/conjure/.env via "
            "`conjure config set`, or in a project-local ./.env[/]"
        )
        return 1
    return 0
