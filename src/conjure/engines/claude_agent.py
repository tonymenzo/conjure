...
from conjure.llm import api_key_present
from conjure.runtime import PERMISSION_WAIT_S
    ...
    def __init__(
        ...
        *,
        ...
        async def can_use_tool(tool_name, args, ctx):  # type: ignore[no-untyped-def]
            ...
            # circuit the prompt. Resets on ``conjure quit``.
        ...
        # which eliminates the per-child ``conjure-mcp`` subprocess
        ...
        if os.environ.get("CONJURE_MCP_INPROC", "1") != "0":
            try:
        ...
        if in_process_server is not None:
            ...
            mcp_bin = shutil.which("conjure-mcp") or "conjure-mcp"
            mcp_servers["spawn"] = McpStdioServerConfig(
                command=mcp_bin,
                env={
                    "CONJURE_TOKEN": record.token,
                    "CONJURE_SOCKET": str(mcp_socket),
        ...
        # ~/.config/conjure/.env for the orchestral engine) leaks
