...
Set ``CONJURE_PROFILE`` to enable. When the value is a directory the
...
value the profile is written to ``./spawn-<name>-<pid>.pstats``.
...
A ``CONJURE_PROFILE_PRINT=1`` env var also dumps the top-30 cumulative
...
def _resolve_target(name: str) -> Path | None:
    ...
    raw = os.environ.get("CONJURE_PROFILE", "").strip()
    if not raw:
        return None
    path = Path(raw)
    pid = os.getpid()
    ...
    if raw in ("1", "true", "yes", "on"):
        return Path.cwd() / f"conjure-{name}-{pid}.pstats"
    # Explicit file path.
...
def profile_session(name: str = "session") -> Iterator[cProfile.Profile | None]:
    """Profile the wrapped block when ``CONJURE_PROFILE`` is set.
    ...
    try:
        ...
        try:
            ...
            pass
        if os.environ.get("CONJURE_PROFILE_PRINT"):
...
def _print_top(prof: cProfile.Profile, name: str, *, rows: int = 30) -> None:
    ...
    sys.stderr.write(f"\n[spawn-profile {name}] top {rows} by cumulative time:\n")
