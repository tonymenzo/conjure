...
The ``conjure-mcp`` subprocess (see :mod:`conjure.mcp_bridge`) was
the v0.1 way to expose conjure's orchestration tools to a
...
# sees as ``mcp__conjure__<DisplayName>`` and matches the names the
...
# Fields present on every conjure tool that the LLM must NOT see —
...
def build_in_process_mcp_server(token: str) -> Any | None:
    ...
    handle) for conjure's orchestration tools, bound to ``token``.
    ...
    return create_sdk_mcp_server(name="conjure", tools=tools)


def _build_input_schema(tool_cls: type) -> dict[str, Any]:
    """Derive the MCP input schema from a conjure tool's pydantic
...
def _make_handler(tool_cls: type, token: str):
    ...
    The conjure tool implementations are synchronous (mailbox ops,
...
def _content(payload: dict[str, Any], *, is_error: bool) -> dict[str, Any]:
    """Wrap a conjure tool's dict result as an MCP tool-call response."""
