"""``conjure-files`` — popup showing every agent's sandbox tree.

Launched via ``tmux display-popup -E "conjure-files --session
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="conjure-files")
    ...
    if socket_path is None or not socket_path.exists():
        print(
            "conjure-files: could not locate a daemon socket "
...
class FilesApp(App):
    ...
    Header { background: #1B4D3E; }
    Footer { background: #1B4D3E; }
    """
