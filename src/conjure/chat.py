"""``conjure-chat`` — per-agent chat TUI.

Each tmux window in a conjure session runs one ``conjure-chat``
...
from conjure.daemon import socket_path_for
from conjure.event_log import tail
from conjure.tui_select import set_mouse_tracking


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="conjure-chat")
    parser.add_argument(
        "--log",
        type=Path,
        required=True,
    ...
    if socket_path is None:
        print(
            "conjure-chat: need --socket or --session",
            file=sys.stderr,
        )
        return 2
