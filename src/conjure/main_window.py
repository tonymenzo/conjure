"""``conjure-main`` — the main TUI window.

The default tmux window (window 0) of every conjure session. A
...
from conjure.chat import ChatView
from conjure.control import ControlClient
from conjure.daemon import list_session_names, socket_path_for
from conjure.status_tree import StatusTree
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="conjure-main")
    parser.add_argument(
        "--socket",
        type=Path,
        default=None,
    ...
    if socket_path is None:
        print(
            "conjure-main: could not locate a daemon socket "
            "(pass --socket or --session)",
            file=sys.stderr,
        )
        return 2
    if not socket_path.exists():
        print(f"conjure-main: socket not found: {socket_path}", file=sys.stderr)
        return 2
...
def _resolve_socket(explicit: Path | None, session: str | None) -> Path | None:
    ...
    if tmux_session and tmux_session.startswith("conjure-"):
...
class MainApp(App):
    ...
    def action_open_in_window(self) -> None:
        ...
        creates the window on first access (running ``conjure-chat``
    ...
    def _spawn_chat_window(self, addr: str, label: str) -> bool:
        """Spawn a ``conjure-chat`` window for ``addr`` in this tmux
        session. Returns True on success."""
        log_path_str = self._log_paths.get(addr)
        if not log_path_str:
            return False
        import shutil as _shutil
        chat_bin = _shutil.which("conjure-chat") or "conjure-chat"
        cmd = " ".join(
