...
The CLI's tmux orchestrator (``combinator run``) uses this to manage
its tmux session lifecycle. The class hides libtmux's deprecation
churn behind a small, intentful API focused on what combinator
actually needs:
...
combinator CLI serialize their usage.
"""
...
class TmuxSession:
    """A combinator tmux session — one per running combinator process."""
