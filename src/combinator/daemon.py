...
``combinator run`` calls ``daemonize`` to detach the runtime from the
controlling terminal. The parent ``combinator run`` invocation returns
...
- ``combinator quit [session]`` reads that PID and sends SIGTERM.
...
- If the daemon dies for any other reason, the PID file is stale; the
  ``combinator run --attach`` and ``combinator quit`` commands handle
  stale files defensively.

This module is deliberately POSIX-only — combinator is a Unix tool.
"""
...
def session_dir() -> Path:
    """Where combinator stores per-session daemon state (PID + log).

    Decoupled from any project's ``store_dir`` so ``combinator quit``
    can find a daemon without needing the original config.
    """
    return Path.home() / ".combinator" / "sessions"
...
def list_session_names() -> list[str]:
    ...
    for p in d.glob("*.pid"):
        name = p.stem
        if not name.startswith("combinator-"):
            continue
