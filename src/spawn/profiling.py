...
Set ``SPAWN_PROFILE`` to enable. When the value is a directory the
...
A ``SPAWN_PROFILE_PRINT=1`` env var also dumps the top-30 cumulative
...
def _resolve_target(name: str) -> Path | None:
    ...
    raw = os.environ.get("SPAWN_PROFILE", "").strip()
    if not raw:
        return None
    path = Path(raw)
    pid = os.getpid()
    ...
    if raw in ("1", "true", "yes", "on"):
        return Path.cwd() / f"spawn-{name}-{pid}.pstats"
    # Explicit file path.
...
def profile_session(name: str = "session") -> Iterator[cProfile.Profile | None]:
    """Profile the wrapped block when ``SPAWN_PROFILE`` is set.
    ...
    try:
        ...
        try:
            ...
            pass
        if os.environ.get("SPAWN_PROFILE_PRINT"):
