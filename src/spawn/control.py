...
socket at ``~/.spawn/sessions/<session>.sock``. The main TUI
(``spawn-main``) and the files popup (``spawn-files``)
...
class ControlServer:
    ...
    def _tool_call(self, request: dict[str, Any]) -> dict[str, Any]:
        """Execute a spawn tool against the runtime, on behalf
        of the token-holding agent. Used by ``spawn-mcp`` to
    ...
    def _sandbox(self, addr_id: str | None, path: str | None) -> dict[str, Any]:
        ...
        from spawn.tools.filesystem import _sandbox_for, _within
...
def _build_tool_registry() -> dict[str, type]:
    ...
    spawn tool classes. Lazily imported so importing
    ``spawn.control`` doesn't drag the whole tool surface in."""
