"""``spawn-main`` — the main TUI window.

The default tmux window (window 0) of every spawn session. A
...
from spawn.chat import ChatView
from spawn.control import ControlClient
from spawn.daemon import list_session_names, socket_path_for
from spawn.status_tree import StatusTree
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="spawn-main")
    parser.add_argument(
        "--socket",
        type=Path,
        default=None,
    ...
    if socket_path is None:
        print(
            "spawn-main: could not locate a daemon socket "
            "(pass --socket or --session)",
            file=sys.stderr,
        )
        return 2
    if not socket_path.exists():
        print(f"spawn-main: socket not found: {socket_path}", file=sys.stderr)
        return 2
...
def _resolve_socket(explicit: Path | None, session: str | None) -> Path | None:
    ...
    if tmux_session and tmux_session.startswith("spawn-"):
...
class MainApp(App):
    ...
    def action_open_in_window(self) -> None:
        ...
        creates the window on first access (running ``spawn-chat``
    ...
    def _spawn_chat_window(self, addr: str, label: str) -> bool:
        """Spawn a ``spawn-chat`` window for ``addr`` in this tmux
        session. Returns True on success."""
        log_path_str = self._log_paths.get(addr)
        if not log_path_str:
            return False
        import shutil as _shutil
        chat_bin = _shutil.which("spawn-chat") or "spawn-chat"
        cmd = " ".join(
