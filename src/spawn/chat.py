"""``spawn-chat`` — per-agent chat TUI.

Each tmux window in a spawn session runs one ``spawn-chat``
...
from spawn.daemon import socket_path_for
from spawn.event_log import tail
from spawn.tui_select import set_mouse_tracking


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="spawn-chat")
    parser.add_argument(
        "--log",
        type=Path,
        required=True,
    ...
    if socket_path is None:
        print(
            "spawn-chat: need --socket or --session",
            file=sys.stderr,
        )
        return 2
