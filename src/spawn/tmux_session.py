...
The CLI's tmux orchestrator (``spawn run``) uses this to manage
its tmux session lifecycle. The class hides libtmux's deprecation
churn behind a small, intentful API focused on what spawn
actually needs:
...
spawn CLI serialize their usage.
"""
...
class TmuxSession:
    """A spawn tmux session — one per running spawn process."""
