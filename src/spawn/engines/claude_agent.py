...
Sync-from-async bridge: the SDK is async, but spawn's driver
...
from spawn.llm import api_key_present
from spawn.runtime import PERMISSION_WAIT_S
...
def _load_default_system_template() -> string.Template:
    ...
    try:
        ...
        text = (
            "You are an agent in the Spawn multi-agent framework.\n\n"
...
class ClaudeAgentEngine:
    ...
    def __init__(
        ...
        *,
        ...
        async def can_use_tool(tool_name, args, ctx):  # type: ignore[no-untyped-def]
            ...
            # circuit the prompt. Resets on ``spawn quit``.
        ...
        # MCP bridge: expose spawn's orchestration surface
        ...
        # which eliminates the per-child ``spawn-mcp`` subprocess
        ...
        if os.environ.get("SPAWN_MCP_INPROC", "1") != "0":
            try:
        ...
        if in_process_server is not None:
            mcp_servers["spawn"] = in_process_server
        elif mcp_socket is not None:
            import shutil

            from claude_agent_sdk.types import McpStdioServerConfig
            mcp_bin = shutil.which("spawn-mcp") or "spawn-mcp"
            mcp_servers["spawn"] = McpStdioServerConfig(
                command=mcp_bin,
                env={
                    "SPAWN_TOKEN": record.token,
                    "SPAWN_SOCKET": str(mcp_socket),
        ...
        # ~/.config/spawn/.env for the orchestral engine) leaks
    ...
    def shutdown(self) -> None:
        ...
        here. Safe to call multiple times — spawn's terminate
