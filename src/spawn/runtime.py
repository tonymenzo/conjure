"""Runtime — the central coordinator of a spawn session.
...
if TYPE_CHECKING:
    from spawn.agent import Engine
...
def _new_async_loop() -> asyncio.AbstractEventLoop:
    ...
    ``SPAWN_UVLOOP=0`` escape hatch is preserved for diagnostics —
    ...
    the runtime."""
    if os.environ.get("SPAWN_UVLOOP", "1") != "0":
        try:
...
class Runtime:
    ...
    def __init__(
        ...
        *,
        ...
        # different ``spawn run`` invocations don't concatenate
        ...
        # Lives in memory only — clears on ``spawn quit``.
    ...
    def get_shared_async_loop(self) -> asyncio.AbstractEventLoop:
        ...
        Uses ``uvloop`` when available unless ``SPAWN_UVLOOP=0``
    ...
    def root(self, spec: AgentSpec) -> Address:
        ...
        with self._lock:
            ...
            if self._root_addr is not None:
                raise SpawnError("root agent already spawned")
    ...
    def list_sessions(store_dir: Path) -> list[str]:
        ...
        ``spawn-<hex>`` and ``auto-<hex>`` naming used at
    ...
    def _require_not_shutdown(self) -> None:
        if self._shutdown:
            raise SpawnError("runtime has been shut down")
    ...
    def _maybe_start_driver(self, record: AgentRecord) -> None:
        ...
        if self._engine_factory is None or record.spec.lazy:
            ...
            return
        from spawn.agent import Agent as AgentWrapper
        from spawn.driver import Driver
