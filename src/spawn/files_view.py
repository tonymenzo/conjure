"""``spawn-files`` — popup showing every agent's sandbox tree.

Launched via ``tmux display-popup -E "spawn-files --session
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="spawn-files")
    ...
    if socket_path is None or not socket_path.exists():
        print(
            "spawn-files: could not locate a daemon socket "
