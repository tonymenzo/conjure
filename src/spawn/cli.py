"""``spawn`` CLI entry point.

Subcommands:

- ``spawn run <config>``   — tmux-native mode: each agent gets a
...
- ``spawn run --attach``   — attach to the most-recent
                                  ``spawn-*`` tmux session.
- ``spawn run --attach <name>`` — attach to a specific session.
- ``spawn repl <config>``  — single-pane REPL mode (no tmux);
                                  useful for scripting and tests.
- ``spawn check <config>`` — validate config + report key
                                  availability.
- ``spawn config list|set|unset`` — manage the user .env.

REPL control commands (``spawn repl`` only):
...
from spawn import _ui
from spawn.address import USER, Address
from spawn.config import load_config
from spawn.control import ControlServer
from spawn.daemon import (
    daemonize,
    is_daemon_running,
    list_session_names,
    log_path_for,
...
from spawn.events import make_system_prompt_event
from spawn.profiling import profile_session
from spawn.record import AgentRecord
from spawn.runner import build_runtime
from spawn.runtime import Runtime
from spawn.tmux_session import TmuxSession, tmux_available
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_p.add_argument(
        ...
        default=None,
        metavar="SESSION",
        help="Attach to an existing spawn session (newest if name omitted).",
    )

    quit_p = sub.add_parser(
        "quit",
        help="Stop a running spawn daemon (newest if name omitted).",
    )
    quit_p.add_argument(
        "session",
        nargs="?",
        default=None,
        help="Session name (default: newest spawn-* daemon).",
    )
...
# ----- spawn run (tmux orchestrator) ------------------------------------

def _cmd_run_tmux(config_path: Path) -> int:
    """Start a daemonized spawn runtime, set up the tmux session
    with one window per agent, then attach the user's terminal.

    Lifecycle:

    - Parent (the original ``spawn run`` invocation) picks a
    ...
    ``spawn run --attach``. Stop the daemon with ``spawn quit``.
    """
    from spawn.env import load_env_files

    console = _ui.make_console()
    if not tmux_available():
        _ui.print_error(
            console,
            "tmux not found on PATH. Install tmux >=3.4 or use `spawn repl`.",
        )
        return 2
    ...
    session_name = f"spawn-{uuid.uuid4().hex[:6]}"
    ...
    if daemon_pid > 0:
        ...
        _ui.print_system(
            ...
            f"[dim]`spawn quit` to stop the daemon[/])",
        )
        time.sleep(0.1)
...
def _run_daemon(*, cfg, session_name: str, pid_path: Path) -> int:
    ...
    Tmux layout is a single window (``spawn-main``) hosting the
    ...
    When ``SPAWN_PROFILE`` is set the whole session is wrapped in
    cProfile and the stats are written out on shutdown — see
    ``spawn.profiling`` for the env var contract.
    """
...
def _run_daemon_inner(*, cfg, session_name: str, pid_path: Path) -> int:
    ...
    store_dir = Path(cfg.runtime.store_dir or "./.spawn/store")
    session_dir = store_dir / "sessions" / session_name
    agents_dir = session_dir / "agents"
    agents_dir.mkdir(parents=True, exist_ok=True)

    # Resolve absolute path to spawn-main so tmux's shell finds it
    # regardless of the user's login-shell PATH (conda envs are rarely
    # on the login PATH).
    import shutil as _shutil
    main_bin = _shutil.which("spawn-main") or "spawn-main"
    ...
    # Single tmux window (spawn-main) hosts everything. Dedicated
...
def _bind_files_popup(session_name: str) -> None:
    ...
    files_path = shutil.which("spawn-files") or "spawn-files"
...
def _cmd_run_attach(session: str) -> int:
    """Attach to an existing spawn tmux session."""
    if not tmux_available():
    ...
    if session == "__most_recent__":
        ...
        if target is None:
            print("no spawn-* tmux session found", file=sys.stderr)
            return 1
        session = target
...
def _find_most_recent_spawn_session() -> str | None:
    """Return the name of the newest live spawn daemon session."""
    ...
    matching = [
        ...
        if (s.session_name or "").startswith("spawn-")
    ]
    if not matching:
        return None
    matching.sort(
...
def _cmd_quit(session: str | None) -> int:
    ...
    if session is None:
        ...
        if session is None:
            _ui.print_error(console, "no spawn daemon found")
            return 1
...
def _cmd_check(config_path: Path) -> int:
    from spawn.env import load_env_files
    from spawn.llm import api_key_present, key_env_for
    ...
    console.print(f"[dim][spawn][/dim] config: [cyan]{config_path}[/]")
    ...
    console.print()
    console.print("[dim][spawn][/dim] LLMs:")
    missing = []
    ...
    if missing:
        ...
        console.print(
            "  [dim]Set them in your shell, in ~/.config/spawn/.env via "
            "`spawn config set`, or in a project-local ./.env[/]"
        )
        return 1
    return 0
