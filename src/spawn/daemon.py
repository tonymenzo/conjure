...
``spawn run`` calls ``daemonize`` to detach the runtime from the
controlling terminal. The parent ``spawn run`` invocation returns
...
- ``spawn quit [session]`` reads that PID and sends SIGTERM.
...
- If the daemon dies for any other reason, the PID file is stale; the
  ``spawn run --attach`` and ``spawn quit`` commands handle
  stale files defensively.

This module is deliberately POSIX-only — spawn is a Unix tool.
"""
...
def session_dir() -> Path:
    """Where spawn stores per-session daemon state (PID + log).

    Decoupled from any project's ``store_dir`` so ``spawn quit``
    can find a daemon without needing the original config.
    """
    return Path.home() / ".spawn" / "sessions"
...
def list_session_names() -> list[str]:
    """Return ``spawn-*`` session names with live PID files."""
    d = session_dir()
    if not d.exists():
        return []
    out: list[str] = []
